<form method="POST" action="{{ $quest ? route('quests.update', $quest->id) : route('quests.store') }}">
    ...
    <div>
        ...
        <input type="text" name="title" id="title"
               class="mt-1 p-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-4xl font-extrabold text-seance-800"
    ...
    <div class="grid grid-cols-1 md:grid-cols-6 gap-4 pt-4">
        <div class="md:col-span-4 space-y-4">
            ...
            <div>
                <label for="intro_text"  class="block text-lg pl-1 pt-0 font-medium text-gray-700"><strong class="text-red">Introduction</strong>
                    (always shown)</label>
                <textarea id="intro_text" name="intro_text"
                          class="tinymce mt-1 rounded-md shadow-sm focus:ring-seance-500 focus:border-seance-500 h-64">{!! old('intro_text', $quest->intro_text ?? '') !!}</textarea>
                @error('intro_text')
                <p class="mt-1 text-sm text-red">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label for="accept_text"
                       class="block text-lg pl-1 pt-0 font-medium text-gray-700"><strong>Accept</strong> (shown before
                    they accept)</label>
                <textarea id="accept_text" name="accept_text"
                          class="tinymce mt-1 rounded-md shadow-sm focus:ring-seance-500 focus:border-seance-500 h-64">{!! old('accept_text', $quest->accept_text ?? '') !!}</textarea>
                @error('accept_text')
                <p class="mt-1 text-sm text-red">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label for="directions_text" class="block text-lg pl-1 pt-0 font-medium text-gray-700"><strong>Directions</strong>
                    (shown after they accept)</label>
                <textarea id="directions_text" name="directions_text"
                          class="tinymce mt-1 rounded-md shadow-sm focus:ring-seance-500 focus:border-seance-500 h-64">{!! old('directions_text', $quest->directions_text ?? '') !!}</textarea>
            ...
            <div>
                <label for="complete_text" class="block text-lg pl-1 pt-0 font-medium text-gray-700"><strong>Complete Text</strong>
                    (shown after they complete)</label>
                <textarea id="complete_text" name="complete_text"
                          class="tinymce mt-1 rounded-md shadow-sm focus:ring-seance-500 focus:border-seance-500 h-64">{!! old('complete_text', $quest->complete_text ?? '') !!}</textarea>
                @error('complete_text')
                <p class="mt-1 text-sm text-red">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="md:col-span-2 space-y-4">
            ...
            <div>
                ...
                <input type="number" name="min_level" id="min_level"
                       ...
                       min="0" value="{{ old('min_level', $quest->min_level ?? '') }}">
                @error('min_level')
                <p class="mt-1 text-sm text-red">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label for="bonus_xp_text" class="block text-lg pl-1 pt-0 font-medium text-gray-700">Bonus XP
                    Instructions</label>
                <textarea id="bonus_xp_text" name="bonus_xp_text"
            ...
            <div>
                <label for="category_id" class="block text-lg pl-1 pt-0 font-medium text-gray-700">Category</label>
                <select name="category_id" id="category_id"
                        class="mt-1 p-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    <option value="">Select Category</option>
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}"
                                @if ($category->id == old('category_id', $quest->category_id ?? '')) selected @endif>{{ $category->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mx-auto">
                <button type="submit" class="tg-button-green" style="width: 100%">
                    {{ $submitButtonText }}
                </button>
            </div>
